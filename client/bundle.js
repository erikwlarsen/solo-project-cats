...
var Gif = __webpack_require__(35);
var ListElement = __webpack_require__(36);
...
var App = function (_Component) {
  ...
  function App(props) {
    ...
    _this.handleClick = function (side) {
      axios.get('http://localhost:3000/gif').then(function (response, error) {
        if (error) console.log(error);
        if (side === 'left') _this.setState({ rightGif: response.data });else if (side === 'right') _this.setState({ leftGif: response.data });
      });

      var gifToSend = void 0;
      if (side === 'left') gifToSend = _this.state.leftGif;else if (side === 'right') gifToSend = _this.state.rightGif;
    ...
    _this.getList = function () {
      axios.get('http://localhost:3000/list').then(function (response, error) {
        _this.setState({ listElements: response.data });
      });
    };
    return _this;
  }
  ...
  _createClass(App, [{
    ...
    value: function componentDidMount() {
      ...
      var rightGif = void 0;
      axios.get('http://localhost:3000/gif').then(function (response, error) {
        if (error) console.log(error);else leftGif = response.data;
        _this2.setState({ leftGif: leftGif });
      });
      axios.get('http://localhost:3000/gif').then(function (response, error) {
        if (error) console.log(error);else rightGif = response.data;
        _this2.setState({ rightGif: rightGif });
      });
    }
  }, {
    key: 'render',
    value: function render() {
      ...
      var gifElements = [React.createElement(Gif, { key: 0, side: 'left', gif: this.state.leftGif, getList: this.getList, handleClick: this.handleClick }), React.createElement(Gif, { key: 1, side: 'right', gif: this.state.rightGif, getList: this.getList, handleClick: this.handleClick })];
...
var ListElement = function (_Component) {
  _inherits(ListElement, _Component);
